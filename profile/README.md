...
| --- | --- | --- | --- |
| **[Kontrat Yöneticisi](https://github.com/Dostlar-Studio/FS25_ContractManager)** · `FS25_ContractManager` | Kontrat üretimini, ödül ve ceza modelini, limitleri sunucu yöneticisinin eline verir. Kontrat ürününü hırsızlığa karşı korur, ortak kontrat açar. | 1.21.1.0 | Yayında |
| **[Discord Bridge](https://bridge.dostlarkonagi.com/)** · `FS25_DiscordBridge` | Sunucu verisini Discord'a ve web panosuna taşır: canlı pano, uyarılar, on beş slash komut, savegame yedeği. Köprü bizim altyapımızda; kurulum tarayıcıdan, dört adım. | 1.3.11.0 | Yayında |
| **AFK Koruması** · `FS25_AFKGuard` | Boşta kalan oyuncuyu önce uyarır, onay gelmezse sunucudan düşürür. Slotu tutan ama oynamayan oyuncuyu temizler. | 1.2.0.0 | Yayında |
| **Uyku Bekçisi** · `FS25_SleepHunter` | Uykuyu saat penceresine hapseder: en erken 20:00'de yatılır, en geç 08:00'de kalkılır. İki katman pencereyi dolanmayı engeller. | 1.2.0.0 | Yayında |
...
| **Oyuncu Pazarı** · `FS25_FarmMarket` | Üretim çıktıları için çiftlikler arası ticaret: satış ilanı, girdi için alış ilanı, ESC menüsünde Pazar sayfası ve haritada işaretler, numaralı fatura. | 1.2.0.0 | Geliştirmede |
...
| --- | --- | --- | --- |
| [FS25_ContractManager](https://github.com/Dostlar-Studio/FS25_ContractManager) | Contract generation, reward and penalty model, limits, server-side product guard, partner contracts | 1.21.1.0 | Released |
| [FS25_DiscordBridge](https://bridge.dostlarkonagi.com/) | Server data in Discord and in a web panel: live dashboard, alerts, fifteen slash commands, savegame backup. We host the bridge; setup runs in the browser | 1.3.11.0 | Released |
| FS25_AFKGuard | Warns idle players, then drops them from the server when no confirmation arrives | 1.2.0.0 | Released |
| FS25_SleepHunter | Confines sleeping to a time window and enforces the wake-up time on every player | 1.2.0.0 | Released |
...
| FS25_FarmMarket | Trade between farms around production: sale listings, buy offers for inputs, a Market page in the ESC menu with map markers, numbered invoices | 1.2.0.0 | In development |
